...
class AccountApisSetUp(APITestCase):
    def setUp(self):
        ...
        self.empty_fields = {"email": "", "username": "", "password": ""}
        ...
        self.normal_user = User.objects.create_user(
            username="testuser", email="testuser@gmail.com", password="testuser1234"
        )
...
class AccountApisAuthTest(AccountApisSetUp):
    ...
    def test_user_account_deletion_without_login(self):
        response = self.client.post(
            ("/accounts/user_delete/1/", {"password": "admin1234"})
        )
        self.assertEqual(response.status_code, 404)
    ...
    def test_fetching_address_details_when_logged_in(self):
        ...
        request = factory.get(
            "/accounts/address-details/1/"
        )  # 1 is the id of the address (created above)
        force_authenticate(request, user=user)
        response = view(request, 1)
        self.assertEqual(response.status_code, 200)
